...
public class TypeChecker {
    ...
    private AFSType ExprType(ExprNode expr) {
        switch(expr) {
            case ExprListDeclaration listDeclaraion -> {
                List<ExprNode> expressions = listDeclaraion.getExpressions();
                

                AFSType firstType = ExprType(expressions.get(0));
                for (ExprNode expression : expressions) {
                    AFSType exprType = ExprType(expression);
                    TypeValidator.validateTypeEquality(exprType, firstType);
                }
                
                expr.setType(firstType);
