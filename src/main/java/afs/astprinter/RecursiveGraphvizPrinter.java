    ...
    private int printEvent(EventNode event) {
        ...
        switch (event) {
            ...
            case EventDeclarationNode node -> {
                ...
                String funIdent = node.getFunIdent();
                int funIdentId = counter++;
                addNode(funIdentId, funIdent);
                addEdge(nodeId, funIdentId);
    ...
    private int printExpr(ExprNode expr) {
        ...
        switch (expr) {
            ...
            case ExprPlaceNode node -> {
                int firstId = printExpr(node.getFirstExpression());
                addEdge(nodeId, firstId);

                int secondId = printExpr(node.getSecondExpression());
                addEdge(nodeId, secondId);

                int thirdId = printExpr(node.getThirdExpression());
                addEdge(nodeId, thirdId);
            }
            case ExprRotateNode node -> {
                ...
                int lastId = printExpr(node.getLastExpression());
                addEdge(nodeId, lastId);
            }
